...
# --------------------------------------------
def imread_uint(path: str | Path, n_channels=3):
...
# matlab's imwrite
# --------------------------------------------
def imsave(img, img_path: str | Path):
...
def calculate_ssim(img1, img2, border=0):
    ...
    if img1.ndim == 2:
        ...
        if img1.shape[2] == 3:
            ...
            result: np.float64 = np.array(ssims).mean()
            return result
        elif img1.shape[2] == 1:
...
def ssim(img1, img2) -> np.float64:
    ...
    ssim_map: np.ndarray = ((2 * mu1_mu2 + C1) * (2 * sigma12 + C2)) / ((mu1_sq + mu2_sq + C1) *
